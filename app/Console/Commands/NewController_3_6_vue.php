...
class NewController_3_6_vue extends Command
{
	/**
	...
	*/
	...
	protected $signature = 'crud_vue_3_6:new ';
  ...
  <div class='card-body'><h1 class=''>Lista de ".$nombrecoNtrol." </h1>  
    ...
    <div class='col-12'>
      ...
      <template v-slot:cell(Acciones)='data'>
          ...
          <b-button-group>
            <b-button v-if=$can('$nombrecoNtrol Editar')$pp  v-b-modal.moda-registro size='sm' variant='warning' @click='editar_registro(data.item)' type='button' class='btn-sm btn btn-wangir mr-1' data-toggle='button' aria-pressed='false' style='margin-bottom: 5px; margin: 5px;'>Editar
            </b-button>
...
export default {
  ...
  methods: {
    ...
    editar_registro(data){//show 
      ...
      axios.get(`'.$nombrecoNtrol.'/'.$peso.'{data.id}`).then(response => {
            const data = response.data;
            if(!response.data){
